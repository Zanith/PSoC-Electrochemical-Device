...
uint8 USB_CheckInput(uint8 buffer[]) {
    if(USBFS_GetEPState(OUT_ENDPOINT) == USBFS_OUT_BUFFER_FULL) {
        //LCD_Position(0,0);
        //LCD_PrintString("checking input");
...
void USB_Export_Data(uint8 array[], uint16 size) {
    //LCD_Position(1,0);
    //sprintf(LCD_str, "array:%d:%d:", array[0], array[1]);
    //LCD_PrintString(LCD_str);
    for (int i=0; i < size; i=i+MAX_BUFFER_SIZE) {
        ...
        //array[i] = size_to_send;
        //LCD_Position(0,0);
        //sprintf(LCD_str, "size2:%d||", size_to_send);
        //LCD_PrintString(LCD_str);
        if(USBFS_GetEPState(IN_ENDPOINT) == USBFS_IN_BUFFER_EMPTY){
            USBFS_LoadEP(IN_ENDPOINT, &array[i], size_to_send);  // TODO: Fix this
            USBFS_EnableOutEP(OUT_ENDPOINT);
        }
    }
//    LCD_Position(1,0);
//    sprintf(LCD_str, "export2:%d", size);
//    LCD_PrintString(LCD_str);
}
